...
-- To optimize this query, we can remove the Card table since the name can be inferred by the type; we use inner joins to avoid too many 'where' conditions; and we use the 'between' operator instead of two separate checks
